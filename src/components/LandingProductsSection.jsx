...
import { collection, getDocs, doc, setDoc, deleteDoc, query, where, orderBy, limit , onSnapshot} from "firebase/firestore";
...
export default function LandingProductsSection() {
  ...
  // Real-time listener using onSnapshot for instant updates without page refresh
  useEffect(() => {
    setLoading(true);

    // Query all products (or sort by createdAt descending)
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      // Filter to only display live products (isLive === true) and limit to 8
      const liveProducts = allProducts
        .filter((product) => product.isLive === true)
        .slice(0, 8);
